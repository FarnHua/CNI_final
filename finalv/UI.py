...
class ClientWindow(QMainWindow):
    ...
    def __init__(
        ...
        self._media_client = client_func(file_name, host_address, host_port, rtp_port)
    ...
    def update_image(self):
        if not self._media_client.is_receiving_rtp:
            return
        frame = None
        if self._media_client.buffer:
            self._media_client.current_frame_number += 1
            frame = self._media_client.buffer.pop(0), self._media_client.current_frame_number
    ...
    def handle_select1(self):
        #First, Teardown
        self._media_client.send_teardown_request()
        self._media_client.rtp_socket.close()
        time.sleep(5)
        #Then, new setup
        print(self.host_address, self.host_port, self.rtp_port)
        self._media_client = client_func("chloe.mjpg", self.host_address, self.host_port, self.rtp_port)
        self._media_client.establish_rtsp_connection()
        self._media_client.send_setup_request()
        self._update_image_timer.start(1000//videostream.fps)
        #New client? 
    ...
    def handle_select2(self):
        ...
        #Then, new setup
        print(self.host_address, self.host_port, self.rtp_port)
        self._media_client = client_func("jf2.mjpg", self.host_address, self.host_port, self.rtp_port)
        self._media_client.establish_rtsp_connection()
        self._media_client.send_setup_request()
        self._update_image_timer.start(1000//videostream.fps)
        #New client? 
    ...
    def handle_select3(self):
        ...
        #Then, new setup
        print(self.host_address, self.host_port, self.rtp_port)
        self._media_client = client_func("4.mjpg", self.host_address, self.host_port, self.rtp_port)
        self._media_client.establish_rtsp_connection()
        self._media_client.send_setup_request()
        self._update_image_timer.start(1000//videostream.fps)
        #New client? 
    ...
    def handle_select_live(self):
        ...
        #Then, new setup
        print(self.host_address, self.host_port, self.rtp_port)
        self._media_client = client_func("livestream", self.host_address, self.host_port, self.rtp_port)
        self._media_client.establish_rtsp_connection()
        self._media_client.send_setup_request()
        self._update_image_timer.start(1000//videostream.fps)
        #New client? 
